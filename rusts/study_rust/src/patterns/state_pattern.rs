...
impl Post {
    ...
    pub fn add_text(&mut self, text: &str) {  //可变
...
//改成，&self.state.as_ref().unwrap().content(self)，也不出错
    ...
    pub fn request_review(&mut self) {
        if let Some(s) = self.state.take() {  //Option.take取出Some，夺权
            //得到特性绑定，调用方法
...
impl State for Draft {
    // --snip--
    fn request_review(self: Box<Self>) -> Box<dyn State> {  //Box<self>,
...
impl State for Published {
    ...
    fn content<'a>(&self, post: &'a Post) -> &'a str {  //指针 -> 指针，生命期
        &post.content  //字符串
    }
}
