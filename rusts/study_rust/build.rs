...
fn main() {
    ...
    let output = Command::new("git").arg("rev-parse").arg("HEAD").output().expect("Failed git execed");
    let commit = String::from_utf8(output.stdout);
    let ci = format!(r#"pub const CURRENT_COMMIT_ID : &'static str = "{}";"#, commit.unwrap_or("invalid!!!".parse().unwrap()));
